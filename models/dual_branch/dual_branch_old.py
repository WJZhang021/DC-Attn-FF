...
class SqueezeAndExcitationBlock(nn.Module):
    def __init__(self, num_channels, ratio=16, dimension=2) -> None:
        super().__init__()
    ...
    def forward(
        ...
        output = torch.mul(input,x) # input (B,C,L) / (B,C,H,W) x (B,C)
...
class Attention(nn.Module):
    # Note that for (batch_size, n_ctx, n_state), attention mechanism is applied on n_ctx dim rather than n_state dim !    
    def __init__(self, n_state: int, n_head: int, dim: str = 'f', dim_num: int = 4):
        super().__init__()
        self.n_head = n_head
        self.dim = dim
        ...
        if not((dim == 't' and dim_num == 4) or dim == 'f' or dim == 'c' or (dim == 'c_p' and dim_num == 4) or dim == 'se'):
        ...
        if self.use_Channel_Attention:
            self.ThrowPot = Channel_Attention(n_state,n_head)
        elif self.use_SE:
            self.ThrowPot = SqueezeAndExcitationBlock(n_state, dimension=(dim_num-2) )
        else:
            self.query = nn.Linear(n_state, n_state)
            self.key = nn.Linear(n_state, n_state, bias=False)
            self.value = nn.Linear(n_state, n_state)
...
class net_gram(nn.Module):
    def __init__(
        ...
        self.res1up = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(3,3), stride=(1,1), padding='same', bias=use_bias),
            Attention(get_nstate(0,attn_dim_up[0]),attn_nhead_up[0],dim=attn_dim_up[0])
            #SqueezeAndExcitationBlock(num_channels=16)
        )
        # add
        self.res1BN = nn.BatchNorm2d(16)
        self.res1down = nn.Sequential(           
            ...
            nn.BatchNorm2d(16),
            nn.Conv2d(in_channels=16, out_channels=16, kernel_size=(3,3), stride=(1,1), padding='same', bias=use_bias),
            Attention(get_nstate(0,attn_dim_down[0]),attn_nhead_down[0],dim=attn_dim_down[0])
            #SqueezeAndExcitationBlock(num_channels=16)
        )
        # add
        
        self.res2BN1 = nn.BatchNorm2d(16) if attn_dim_btw[0]=='none' else nn.Sequential(
            Attention(get_nstate(0,attn_dim_btw[0]),attn_nhead_btw[0],dim=attn_dim_btw[0]),
            nn.BatchNorm2d(16)
        )
        self.res2up = nn.Sequential(
            ...
            Attention(get_nstate(1,attn_dim_up[1]),attn_nhead_up[1],dim=attn_dim_up[1])
        ...
        # add
        self.res2BN2 = nn.BatchNorm2d(32)
        self.res2down = nn.Sequential(           
            ...
            nn.ReLU(),            
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(3,3), stride=(1,1), padding='same', bias=use_bias),
            Attention(get_nstate(1,attn_dim_down[1]),attn_nhead_down[1],dim=attn_dim_down[1])
            #SqueezeAndExcitationBlock(num_channels=32)
        )
        # add
        
        self.res3BN1 = nn.BatchNorm2d(32) if attn_dim_btw[1]=='none' else nn.Sequential(
            Attention(get_nstate(1,attn_dim_btw[1]),attn_nhead_btw[1],dim=attn_dim_btw[1]),
            nn.BatchNorm2d(32)
        )
        self.res3up = nn.Sequential(
            ...
            Attention(get_nstate(2,attn_dim_up[2]),attn_nhead_up[2],dim=attn_dim_up[2])
        ...
        # add
        self.res3BN2 = nn.BatchNorm2d(64)
        self.res3down = nn.Sequential(           
            ...
            nn.ReLU(),            
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3,3), stride=(1,1), padding='same', bias=use_bias),
            Attention(get_nstate(2,attn_dim_down[2]),attn_nhead_down[2],dim=attn_dim_down[2])
            #SqueezeAndExcitationBlock(num_channels=64)
        )
        # add
        
        self.res4BN1 = nn.BatchNorm2d(64) if attn_dim_btw[2]=='none' else nn.Sequential(
            Attention(get_nstate(2,attn_dim_btw[2]),attn_nhead_btw[2],dim=attn_dim_btw[2]),
            nn.BatchNorm2d(64)
        )
        self.res4up = nn.Sequential(
            ...
            Attention(get_nstate(3,attn_dim_up[3]),attn_nhead_up[3],dim=attn_dim_up[3])
        ...
        # add
        self.res4BN2 = nn.BatchNorm2d(128)
        self.res4down = nn.Sequential(           
            ...
            nn.ReLU(),            
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3,3), stride=(1,1), padding='same', bias=use_bias),
            Attention(get_nstate(3,attn_dim_down[3]),attn_nhead_down[3],dim=attn_dim_down[3])
            #SqueezeAndExcitationBlock(num_channels=128)
        )
        # add
        ...
        self.pool = nn.Sequential(
            ...
            Attention(get_nstate(3,attn_dim_btw[3]),attn_nhead_btw[3],dim=attn_dim_btw[3]),
            nn.Identity() if attn_dim_btw[4]=='none' else 
            Attention(get_nstate(3,attn_dim_btw[4]),attn_nhead_btw[4],dim=attn_dim_btw[4]),
            nn.Identity() if attn_dim_btw[5]=='none' else 
            Attention(get_nstate(3,attn_dim_btw[5]),attn_nhead_btw[5],dim=attn_dim_btw[5]),
